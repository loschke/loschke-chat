...
export function ModelEditor({ modelId, onSuccess }: ModelEditorProps) {
    ...
    async function loadModel() {
      try {
        ...
        if (res.ok) {
          ...
          // Format model data as editable JSON (exclude DB-internal fields)
          const editable = {
          ...
          setMessage("Model konnte nicht geladen werden")
          setStatus("error")
        }
      } catch {
  ...
  if (loading) {
    return <p className="text-sm text-muted-foreground">Lade Model...</p>
  }

  return (
    <div className="space-y-4">
      <div className="space-y-2">
        <label className="text-sm font-medium">Model-Konfiguration (JSON)</label>
