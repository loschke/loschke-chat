...
export function CodePreview({ code, language }: CodePreviewProps) {
  ...
  useEffect(() => {
    ...
    if (
      prevRef.current.code === code &&
      prevRef.current.language === lang &&
      html
    )
      return;
    ...
    async function highlight() {
      try {
        ...
        // Sanitize the HTML to prevent XSS while allowing Shiki's styling
        const sanitizedResult = xss(result, {
          whiteList: {
        ...
        if (!cancelled) setHtml(sanitizedResult);
      } catch (err) {
