 ...
 * HTML is created server-side. Returns null on error.
 */
export const extractDesignFromToolPart: (part: { type: string; [key: string]: unknown }) => ExtractorResult = (() => {
  const generateExtractor = createToolPartExtractor({
    toolName: "generate_design",
    mode: "server-side",
    mapInput: (input, output) => {
      if (output) {
        return {
          title: (output.title as string) ?? (input.title as string) ?? "UI Design",
          type: "html",
        }
      }
      return {
        title: (input.title as string) ?? "Design wird generiert\u2026",
        type: "html",
      }
    },
    shouldSkipOutput: (output) => !!(output?.error),
  })

  const editExtractor = createToolPartExtractor({
  ...
  return (part) => generateExtractor(part) ?? editExtractor(part)
})()
